...
export function Navbar({ user }: { user: User | null }) {
  ...
  return (
    <header className="sticky top-0 z-10 border-b border-gray-200 bg-white/90 py-3 shadow-sm backdrop-blur sm:py-4">
      <nav className="app-container flex flex-wrap items-center justify-between gap-x-6 gap-y-3">
        ...
        <div className="flex w-full items-center gap-5 border-t border-gray-100 pt-3 sm:hidden">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
